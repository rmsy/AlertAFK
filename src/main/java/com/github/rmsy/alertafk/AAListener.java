...
public class AAListener implements Listener {
    ...
    public void onPlayerConnect(PlayerJoinEvent e) {
        ...
        AlertAFK.plugin.aaPlayers.put(e.getPlayer().getName(), new SimplePlayer(player));
    }

    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onPlayerDisconnect(PlayerQuitEvent e) {
        Player player = e.getPlayer();
        SimplePlayer simplePlayer = (SimplePlayer) AlertAFK.plugin.aaPlayers.get(player.getName());
        SimpleConfigManager.savePlayerAliases(AlertAFK.plugin, simplePlayer);
        simplePlayer.aliases.clear();
        simplePlayer.aliases = null;
    ...
    public void onPlayerChat(AsyncPlayerChatEvent e) {
        ...
        if (e.isAsynchronous()) {
            ...
            for (Player onlinePlayer : onlinePlayers) {
                simplePlayer = (SimplePlayer) AlertAFK.plugin.aaPlayers.get(onlinePlayer.getName());
                if (e.getMessage().contains(onlinePlayer.getName())) {
                    if (simplePlayer.afk) {
                        player.sendMessage(ChatColor.RED + "Note: " + onlinePlayer.getDisplayName() + ChatColor.RED + " is AFK - " + simplePlayer.afkMessage);
                    }
                } else {
                    if (simplePlayer.aliases.toArray().length > 0) {
                        for (Object alias : simplePlayer.aliases.toArray()) {
